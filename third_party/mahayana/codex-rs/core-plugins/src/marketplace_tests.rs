...
#[test]
fn find_marketplace_plugin_supports_root_marketplace_layout() {
    let tmp = tempdir().unwrap();
    let marketplace_root = tmp.path().join("bundled-marketplace");
    fs::create_dir_all(marketplace_root.join("plugins/local-plugin")).unwrap();
    fs::write(
  ...
  "plugins": [
    ...
    let resolved = find_marketplace_plugin(
        &AbsolutePathBuf::try_from(marketplace_root.join("marketplace.json")).unwrap(),
        "local-plugin",
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        MarketplacePluginSource::Local {
            path: AbsolutePathBuf::try_from(marketplace_root.join("plugins/local-plugin")).unwrap(),
        }
    );
}
