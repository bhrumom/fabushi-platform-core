...
pub fn home_html(plugin_id: &str) -> Result<String, String> {
    ...
    if plugin_id == "global-dharma" {
        return Ok(global_dharma_home_html(&app));
    }
    let buttons = app
        .tools
        .iter()
...
fn global_dharma_home_html(app: &AppDefinition) -> String {
    let template = r#"<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; connect-src 'none'"><style>
...
mod tests {
    ...
    #[test]
    fn global_dharma_home_ui_is_graphical_and_file_aware() {
        let html = home_html("global-dharma").unwrap();
        assert!(html.contains("data-testid=\"global-dharma-file-picker\""));
        assert!(html.contains("file.text()"));
        assert!(html.contains("发送到全球法布施"));
        assert!(html.contains("method:'tools/call'"));
        assert!(html.contains("arguments:{content:text,task_id:'mahayana'}"));
        assert!(html.contains("const isResponse="));
        assert!(!html.contains("FabushiMiniApp"));
    }
